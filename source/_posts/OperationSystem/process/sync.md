...
由于信号量机制的每次同步操作都需要调用 wait(S)，signal(S)，给管理带来了麻烦，还可能因为操作不当导致死锁，所以引入了进的进程同步工具——管程

#### 定义

代表共享资源的数据结构，以及由对该共享数据结构实施操作的一组过程所组成的资源管理程序，共同组成了一个操作系统的资源管理模块
...
- 局部于管程内部的共享数据结构说明
- 对该数据结构进行操作的过程
- 对局部于管程内部的共享数据的设置初始值的语句

![monitors](/Users/H/Documents/GitHub/blog.DoubleWoodH.club/source/_posts/OperationSystem/process/images/monitors.png)
...
局部于管程内的数据结构，仅能被内部的过程访问；内部的过程也仅能调用内部的数据结构。所有进程要访问临界资源时，都必须经过管程。而管程只允许一个进程进入，从而实现了进程互斥
...
- 二者都定义了数据结构，但进程定义的是私有的 (PCB)，而管程定义的是公有的，如消息队列等
- 二者都存在对数据结构的操作，但进程是由顺序程序执行的操作，而管程进行同步和初始化操作
- 进程是为了实现并发性，而管程是为了解决共享资源的互斥问题
- 进程通过调用管程的过程对共享数据结构进行操作，就像调用子程序一样，因而管程是被动工作方式，而进程是主动工作方式
- 进程间能并发执行，而管程与其调用者不行
- 进程具有动态性，由“创建”而诞生，由“撤销”而消亡，而管程则是系统中的一个资源管理模块，供进程调用
