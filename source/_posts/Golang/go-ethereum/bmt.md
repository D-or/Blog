...
func (self *Tree) Draw(hash []byte, d int) string {
	...
	var anc []*Node
  	// left: 每一节点的左节点哈希值的集合的切片
    // right: 每一节点的右节点哈希值的集合的切片
	...
	anc = self.leaves
  	// hashes -> [[第1层到第(level - 1)层的 hash], [第1层到第(level - 2)层的 hash], ···, [第1层级的 hash]]
	var hashes [][]string
	for l := 0; len(anc) > 0; l++ {
		var nodes []*Node
		hash := []string{""}
      	// hash -> ["", "左节点哈希值|右节点哈希值", ···, ""]  从第一层到当前层
		for i, n := range anc {
			hash = append(hash, fmt.Sprintf("%v|%v", hashstr(n.left), hashstr(n.right)))
			// 将每个左节点的父节点插入 nodes 中
  	...
  	// hashes -> [[第1层到第(level - 1)层的 hash], [第1层到第(level - 2)层的 hash], ···, [第1层级的 hash], ["", 输入的 hash 值参数, ""]]
	hashes = append(hashes, []string{"", fmt.Sprintf("%v", hashstr(hash)), ""})
	total := 60
  	// del -> 长度为 29
	del := "                             "
	var rows []string
	// rows -> ["0: ", "1: ", "2: ", ··· , "len(hashes) - 2: ", "len(hashes) - 1: "]
	for i := len(hashes) - 1; i >= 0; i-- {
		var textlen int
		hash := hashes[i]
      	// textlen -> 每一层级 hash 的字符串长度总和
		for _, s := range hash {
			textlen += len(s)
		}
		if total < textlen {
	...
	// rows -> ["0: ", "1: ", "2: ", ··· , "len(hashes) - 2: ", "len(hashes)
  	// - 1: ", ?]
